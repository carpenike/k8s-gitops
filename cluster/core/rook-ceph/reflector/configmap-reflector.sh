...
for configmap in ${configmaps}; do
    configmap_source_content="$(kubectl get configmap "${configmap}" -n "${namespace_source}" -o json | jq 'del(.metadata.managedFields, .metadata.creationTimestamp, .metadata.resourceVersion, .metadata.uid)')"
    ...
    for namespace in ${namespace_destination}; do
        if kubectl get configmap "${configmap}" -n "${namespace}" >/dev/null 2>&1; then
            configmap_dest_content="$(kubectl get configmap "${configmap}" -n "${namespace}" -o json | jq 'del(.metadata.managedFields, .metadata.creationTimestamp, .metadata.resourceVersion, .metadata.uid)')"
